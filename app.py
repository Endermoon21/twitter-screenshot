...
def capture_tweet_playwright(url, parsed, theme="dark", hide_metrics=False, width=550, include_replies=False, max_replies=5, reply_tweet_ids=None):
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # -------------------------------------------------------------
            # Expand a quoted/embedded ("repost") tweet to its FULL text.
            # X truncates the quoted tweet inside a quote-tweet (e.g. 279 of
            # 528 chars) and makes the card a click-through link. We detect the
            # quoted tweet's status id, fetch the original tweet's full text in
            # a separate page, and inject it into the embed so the screenshot
            # shows the complete original. Best-effort: any failure leaves the
            # (truncated) embed as-is and capture continues normally.
            # -------------------------------------------------------------
            try:
                quoted_id = page.evaluate('''(mainId) => {
                    const article = document.querySelector('article');
                    if (!article) return null;
                    // The quoted tweet is an inner div[role="link"] that contains
                    // its own tweetText and a /status/ link with a different id.
                    const cards = article.querySelectorAll('div[role="link"]');
                    for (const card of cards) {
                        if (!card.querySelector('[data-testid="tweetText"]')) continue;
                        const links = card.querySelectorAll('a[href*="/status/"]');
                        for (const a of links) {
                            const m = a.getAttribute('href').match(/\\/status\\/(\\d+)/);
                            if (m && m[1] !== mainId) return m[1];
                        }
                    }
                    return null;
                }''', parsed['tweet_id'])
                ...
                if quoted_id:
                    logger.info(f"Quoted tweet detected (id={quoted_id}); fetching full original text")
                    orig_page = context.new_page()
                    orig_html = None
                    orig_len = 0
                    try:
                        orig_page.goto(f"https://x.com/i/status/{quoted_id}",
                                       wait_until="domcontentloaded", timeout=30000)
                        orig_page.wait_for_timeout(3500)
                        orig_data = orig_page.evaluate('''() => {
                            const article = document.querySelector('article');
                            if (!article) return null;
                            const t = article.querySelector('[data-testid="tweetText"]');
                            if (!t) return null;
                            return {html: t.innerHTML, len: (t.textContent || '').length};
                        }''')
                        if orig_data:
                            orig_html = orig_data['html']
                            orig_len = orig_data['len']
                    except Exception as e:
                        logger.warning(f"Could not load original quoted tweet: {e}")
                    finally:
                        orig_page.close()

                    if orig_html:
                        # Only replace if the original is meaningfully longer than
                        # the (X-truncated) embed — otherwise just unclamp below.
                        replaced = page.evaluate('''({origHtml, origLen}) => {
                            const article = document.querySelector('article');
                            if (!article) return false;
                            const texts = article.querySelectorAll('[data-testid="tweetText"]');
                            if (texts.length < 2) return false;
                            const embed = texts[texts.length - 1];  // quoted text is last
                            const embedLen = (embed.textContent || '').length;
                            if (origLen > embedLen + 5) {
                                embed.innerHTML = origHtml;
                                return true;
                            }
                            return false;
                        }''', {'origHtml': orig_html, 'origLen': orig_len})
                        if replaced:
                            logger.info(f"Injected full original text ({orig_len} chars) into quoted embed")
            except Exception as e:
                logger.warning(f"Quote-expansion step failed (continuing): {e}")

            # Unclamp ALL tweet text (main + quoted). X line-clamps long text via
            # -webkit-line-clamp on display:-webkit-box; switching to display:block
            # and removing the clamp/max-height reveals the full text. This also
            # makes the freshly-injected original text render in full.
            page.evaluate('''() => {
                const article = document.querySelector('article');
                if (!article) return;
                const unclamp = (el) => {
                ...
                article.querySelectorAll('[data-testid="tweetText"]').forEach(unclamp);
                // Catch any other element still carrying a webkit line clamp
                article.querySelectorAll('*').forEach(el => {
                    const cs = getComputedStyle(el);
                    if (cs.webkitLineClamp && cs.webkitLineClamp !== 'none') unclamp(el);
                });
            }''')
            # Give injected emoji/images a moment to load before measuring
            page.wait_for_timeout(600)

            # Expanded quote text can push content past the viewport; grow the
            # viewport so the clip screenshot captures the full article (+ any
            # visible replies) instead of cutting off.
            try:
                needed = page.evaluate('''() => {
                    let maxBottom = 0;
                    const a = document.querySelector('article');
                    if (a) maxBottom = a.getBoundingClientRect().bottom;
                    (window.__replyArticles || []).forEach(r => {
                        const b = r.getBoundingClientRect().bottom;
                        if (b > maxBottom) maxBottom = b;
                    });
                    return Math.ceil(maxBottom);
                }''')
                target_h = min(6000, max(1200, int(needed) + 400))
                page.set_viewport_size({"width": width, "height": target_h})
                page.wait_for_timeout(300)
            except Exception as e:
                logger.warning(f"viewport resize skipped: {e}")
